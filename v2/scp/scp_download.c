...
int verbosity=0;
const char *createcommand="rm -fr /tmp/libssh_tests && mkdir /tmp/libssh_tests && cd /tmp/libssh_tests && date > a && date > b && mkdir c && date>c/e && date > d";
char *host=NULL;
...
static int fetch_files(ssh_session session){
  ...
  do {
	  ...
	  case SSH_SCP_REQUEST_NEWFILE:
          ...
          char aaa[100];
          strncpy(aaa, buffer, 90);
          aaa[90]='\0';
          printf("content: %s, read count: %d\n", aaa, r);
		  if(r==SSH_ERROR){
